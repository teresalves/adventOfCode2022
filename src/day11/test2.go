...
func main() {  
	...
	// Process everything
	commonDivisor := calcMaxCommonDivisor(monkeys)
	...
	for i:= 0; i < numberOfRounds; i++ {
		...
		for monkey := 0; monkey < numberOfMonkeys; monkey++{
			for _,item := range monkeys[monkey].items {
				...
				monkeys[newMonkey].items = append(monkeys[newMonkey].items, value % commonDivisor)
			}
...
func calcMaxCommonDivisor(monkeys []Monkey) int{
	result := 1
	for _, value := range monkeys {
		result*=value.divisorTest
	}
	return result
}